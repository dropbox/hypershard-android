package com.dropbox.mobile.kastree.ast.psi
...
object Corpus {
    ...
    val kotlinRepoTestData by lazy {
        if (System.getenv("KOTLIN_REPO") == null) {
            println("NOTE: to do a full test, define KOTLIN_REPO w/ a checkout of the full kotlin repository.")
            ArrayList<Path>()
        } else {
    ...
    val localTestData by lazy {
        loadTestDataFromDir(File(javaClass.getResource("/kastreeTestData").toURI()).toPath())
    }
    ...
    fun loadTestDataFromDir(root: Path) = Files.walk(root).filter { it.toString().endsWith(".kt") }.toList().map {
        ...
        Unit.FromFile(
                ...
                errorMessages = overrideErrors[relativePath]
                        ?: Paths.get(it.toString().replace(".kt", ".txt")).let {
